    ...
    <div id="body">
        <div class="add_list">
            ...
            <div class="item flexbox align-items">
                ...
                <div class="right flex">
                    <input type="text" value="" placeholder="请输入" v-model="mobile" maxlength="11">
                </div>
            </div>
            <div class="item flexbox align-items">
...
export default {
    ...
    methods: {
        commit () {
            if (this.verify()) {
                let obj = {
                    expressName: this.cneeName, // 收件人
                    expressMobile: this.mobile, // 联系方式
                    areaCode: this.areaCode, // 邮政编码
                    expressProvince: this.arr[0], // 所在省
                    expressCity: this.arr[1], // 所在市
                    expressDistrict: this.areaId, // 所在地区
                    expressAddressDetail: this.areaName + this.address, //所在地区
                    expressAddress: this.address // 详细地址
                }
                storage.set('location', JSON.stringify(obj));
                window.history.back();
